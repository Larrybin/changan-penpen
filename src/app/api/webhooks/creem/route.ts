...
import type { CreemWebhookEvent, CreemCheckout, CreemSubscription } from "@/modules/creem/models/creem.types";
import { createOrUpdateCustomer, createOrUpdateSubscription, addCreditsToCustomer, getCustomerIdByUserId } from "@/modules/creem/services/billing.service";
...
export async function POST(request: Request) {
    try {
        ...
        if (env.CREEM_LOG_WEBHOOK_SIGNATURE === "1" && (process.env.NODE_ENV !== "production")) {
...
async function onCheckoutCompleted(checkout: CreemCheckout) {
    if (!checkout?.metadata?.user_id) throw new Error("Missing user_id in metadata");
    ...
    if (checkout?.metadata?.product_type === "credits") {
        ...
        await addCreditsToCustomer(customerId, credits, orderId, `Purchased ${credits} credits`);
        return;
    }
...
async function onSubscriptionChanged(subscription: CreemSubscription) {
    const metaUserId = (subscription?.metadata as any)?.user_id as string | undefined;

    let customerId: number | null = null;
    if (typeof subscription.customer === "object" && subscription.customer?.id) {
        const userId = metaUserId || "";
        customerId = await createOrUpdateCustomer(subscription.customer as any, userId);
    } else if (metaUserId) {
        customerId = await getCustomerIdByUserId(metaUserId);
    }
    if (!customerId) throw new Error("Cannot resolve customer for subscription event");
