...
import { getActiveAppLocales } from '@/lib/seo';
...
export async function GET(): Promise<MetadataRoute.Sitemap> {
  try {
    ...
    // 确定基础URL
    const baseUrl = settings.siteUrl?.trim() &&
      (settings.siteUrl.startsWith('http://') || settings.siteUrl.startsWith('https://'))
      ? settings.siteUrl.trim()
      : envAppUrl || 'https://banana-generator.com';
