 ...
 * 确保URL有正确的格式
 */
function normalizeUrl(url: string, addTrailingSlash: boolean = false): string {
    try {
        const urlObj = new URL(url);
...
export function validateCanonicalUrl(url: string): {
    ...
    try {
        ...
        // 检查查询参数
        if (urlObj.search) {
            const hasTrackingParams = /[?&](utm_|fbclid|gclid|msclkid)/.test(
                urlObj.search,
            );
        ...
        return {
            isValid: false,
            errors: [
                `无效的URL格式: ${error instanceof Error ? error.message : "未知错误"}`,
            ],
