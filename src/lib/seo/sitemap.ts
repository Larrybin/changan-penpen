...
export function getSitemapStats(sitemap: MetadataRoute.Sitemap): {
    ...
    sitemap.forEach((entry) => {
        ...
        stats.urlsByChangeFrequency[freq] =
            (stats.urlsByChangeFrequency[freq] || 0) + 1;
    ...
    stats.averagePriority =
        sitemap.length > 0 ? totalPriority / sitemap.length : 0;
    stats.locales = Array.from(localeSet);
 ...
 * 生成站点地图配置建议
 */
export function getSitemapRecommendations(
