...
export function getBreadcrumbConfig(
  ...
  switch (normalizedPath) {
      ...
      config.items = [
        {
          ...
          url: buildLocalizedPath(locale, "/about"),
        },
      ];
      break;
      ...
      config.items = [
        {
          ...
          url: buildLocalizedPath(locale, "/contact"),
        },
      ];
      break;
      ...
      config.items = [
        {
          ...
          url: buildLocalizedPath(locale, "/privacy"),
        },
      ];
      break;
      ...
      config.items = [
        {
          ...
          url: buildLocalizedPath(locale, "/terms"),
        },
      ];
      break;
      ...
      config.items = [
        {
          ...
          url: buildLocalizedPath(locale, "/dashboard"),
        },
      ];
      break;
      ...
      config.items = [
        {
          ...
          url: buildLocalizedPath(locale, "/billing"),
        },
      ];
      break;
    ...
    default:
      // 动态路由处理
      if (pathSegments[0] === "dashboard" && pathSegments[1] === "todos") {
        config.items = [
          {
            name: t("Dashboard.title") || "仪表板",
            url: buildLocalizedPath(locale, "/dashboard"),
        ...
        if (pathSegments[2] === "edit") {
          config.items.push({
            ...
            url: buildLocalizedPath(locale, `/dashboard/todos/${pathSegments[3]}/edit`),
          });
        ...
        if (pathSegments[1]) {
          ...
          if (adminPageMap[pathSegments[1]]) {
            config.items.push({
              ...
              url: buildLocalizedPath(locale, `/admin/${pathSegments[1]}`),
            });
          }
        }
      }
...
export function generateBreadcrumbSchema(
  ...
  if (config.showHome) {
    items.push({
      ...
      item: buildLocalizedPath(locale, "/"),
    });
  }
...
export function generateBreadcrumbDataAttributes(config: BreadcrumbConfig) {
  return {
    ...
    "data-show-home": config.showHome.toString(),
  };
}
...
export class BreadcrumbUtils {
  ...
  static getCurrentPage(config: BreadcrumbConfig): BreadcrumbItem | null {
    if (config.items.length === 0) {
      return config.showHome ? { name: config.homeName || "首页", url: "/" } : null;
    }
   ...
   * 格式化面包屑为JSON-LD字符串
   */
  static toJSONLD(config: BreadcrumbConfig, locale: string, siteSettings?: any): string {
