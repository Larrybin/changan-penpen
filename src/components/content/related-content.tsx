...
import { ArrowRightIcon, BookOpenIcon, UsersIcon, SettingsIcon, FileTextIcon, TagIcon } from "lucide-react";
...
export function RelatedContent({
  ...
  const cardClasses = {
    default: "group flex flex-col space-y-2 rounded-lg border p-6 transition-all hover:shadow-md hover:border-primary/50",
    bordered: "group flex flex-col space-y-2 rounded-lg border-2 border-border bg-background p-6 transition-all hover:border-primary/50",
    elevated: "group flex flex-col space-y-2 rounded-lg border bg-background shadow-sm p-6 transition-all hover:shadow-md hover:border-primary/50",
  };

  return (
    <section className={containerClasses} aria-labelledby="related-content-title">
      {title && (
...
function generateRelatedContent(
  ...
  const contentMap: Record<string, RelatedContentItem[]> = {
    "": [
      {
        title: "功能介绍",
        description: "了解我们产品的核心功能，包括任务管理、团队协作和数据分析等模块。",
        href: "/features",
        category: "功能",
        tags: ["任务管理", "团队协作", "数据分析"],
        readTime: "5分钟阅读",
      ...
      {
        title: "价格方案",
        description: "选择适合您需求的价格方案，灵活订阅，随时升级或降级。",
        href: "/pricing",
        category: "产品",
        tags: ["订阅", "价格", "方案"],
        readTime: "3分钟阅读",
      ...
      {
        title: "用户指南",
        description: "详细的使用指南，帮助您快速上手并充分利用所有功能。",
        href: "/docs",
        category: "教程",
        tags: ["指南", "教程", "帮助"],
        readTime: "8分钟阅读",
    ...
    "contact": [
      ...
      {
        title: "技术支持",
        description: "专业技术支持团队为您提供及时的技术帮助和问题解决。",
        href: "/support",
        category: "功能",
        tags: ["支持", "技术", "帮助"],
        readTime: "3分钟阅读",
...
export class RelatedContentUtils {
   ...
   * 根据页面类型获取推荐内容
   */
  static getRecommendations(pageType: string, limit: number = 6): RelatedContentItem[] {
    const preset = RelatedContentPresets[pageType as keyof typeof RelatedContentPresets];
   ...
   * 根据标签获取相关内容
   */
  static getByTags(tags: string[], items: RelatedContentItem[]): RelatedContentItem[] {
   ...
   * 根据分类获取相关内容
   */
  static getByCategory(category: string, items: RelatedContentItem[]): RelatedContentItem[] {
   ...
   * 验证内容项
   */
  static validateItem(item: RelatedContentItem): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
