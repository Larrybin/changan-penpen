...
export function Breadcrumb({
  ...
  useEffect(() => {
    ...
    try {
      const config = getBreadcrumbConfig(pathname, currentLocale, t, mockSettings);
      const items = config.showHome && showHome
        ? [{ name: config.homeName || "首页", url: "/" }, ...config.items]
        : config.items;
      ...
      if (schema && BreadcrumbUtils.shouldShowBreadcrumbs(pathname)) {
        const breadcrumbSchema = generateBreadcrumbSchema(config, currentLocale, mockSettings);

        // 创建或更新结构化数据script标签
        let scriptTag = document.getElementById('breadcrumb-schema') as HTMLScriptElement;
        if (!scriptTag) {
...
export function BreadcrumbItem({
  ...
  return (
    <li className="flex items-center">
      {!isFirst && (
        <ChevronRightIcon className="w-4 h-4 mx-1 text-muted-foreground/50" aria-hidden="true" />
      )}
      <Link
        href={href}
        className={cn(
...
export const BreadcrumbStyles = {
  ...
  // 卡片样式
  card: "flex items-center space-x-1 text-sm bg-muted/30 px-3 py-2 rounded-md border",
...
// 面包屑验证工具
export class BreadcrumbValidator {
  /**
   * 验证面包屑结构
   */
  static validateBreadcrumb(breadcrumb: Array<{ name: string; url: string }>) {
    const errors: string[] = [];
    ...
    breadcrumb.forEach((item, index) => {
      try {
        ...
        errors.push(`面包屑项目${index + 1}: "${item.url}" 不是有效的URL`);
      }

      if (!item.name || item.name.trim().length === 0) {
        errors.push(`面包屑项目${index + 1}: 名称不能为空`);
      }
    });

    // 检查层级深度
    if (breadcrumb.length > 5) {
      warnings.push("面包屑层级过深，建议控制在5层以内");
    }
   ...
   * 获取面包屑SEO建议
   */
  static getSeoSuggestions(breadcrumb: Array<{ name: string; url: string }>) {
    const suggestions: string[] = [];

    if (breadcrumb.length === 0) {
      suggestions.push("考虑为重要页面添加面包屑导航");
    }

    // 检查关键词优化
    breadcrumb.forEach((item, index) => {
      if (item.name.length > 50) {
        suggestions.push(`面包屑项目${index + 1} "${item.name}" 过长，建议缩短至50字符以内`);
      }
    });

    // 检查内部链接
    const internalLinks = breadcrumb.filter(item =>
      item.url.startsWith('/') || item.url.startsWith(window.location?.origin || '')
    );

    if (internalLinks.length !== breadcrumb.length) {
      suggestions.push("面包屑应全部链接到内部页面");
    }
