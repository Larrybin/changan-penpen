...
export function WebVitals() {
  ...
  const handleWebVital = useCallback((metric: any) => {
    // 更新本地状态
    ...
    switch (metric.name) {
      case "LCP":
        if (metric.rating !== "good") {
          console.warn(`⚠️ LCP性能警告: ${metric.value.toFixed(2)}ms (${metric.rating})`);
        }
        break;
      case "INP":
        if (metric.rating !== "good") {
          console.warn(`⚠️ INP性能警告: ${metric.value.toFixed(2)}ms (${metric.rating})`);
        }
        break;
      case "CLS":
        if (metric.rating !== "good") {
          console.warn(`⚠️ CLS性能警告: ${metric.value.toFixed(4)} (${metric.rating})`);
        }
        break;
    }
  }, [sendToAnalytics]);
...
export function calculatePerformanceScore(metrics: WebVitalsMetrics[]): number {
  ...
  const availableCoreMetrics = metrics.filter(m => coreMetrics.includes(m.name));
...
export function useWebVitals() {
  ...
  const updateMetrics = useCallback((newMetric: WebVitalsMetrics) => {
    setMetrics(prev => {
      const existingIndex = prev.findIndex(m => m.name === newMetric.name);
      if (existingIndex >= 0) {
        const updated = [...prev];
        updated[existingIndex] = newMetric;
        return updated;
