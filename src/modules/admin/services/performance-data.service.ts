...
type HealthComponentInput<T> = Partial<T>;
type HealthDataInput = Partial<SystemHealthData> & {
    components?: {
        database?: HealthComponentInput<
            SystemHealthData["components"]["database"]
        >;
        cache?: HealthComponentInput<SystemHealthData["components"]["cache"]>;
        api?: HealthComponentInput<SystemHealthData["components"]["api"]>;
        storage?: HealthComponentInput<
            SystemHealthData["components"]["storage"]
        >;
    };
};
...
export class PerformanceDataService {
    private cache = new Map<
        string,
        { data: PerformanceMetrics; timestamp: number }
    >();
    ...
    private async collectPerformanceData(
        ...
        // 并行收集各类数据
        const [webVitalsData, systemHealthData, quickStatsData] =
        ...
        const overview = this.calculateOverallScores(
            webVitalsData.status === "fulfilled"
                ? webVitalsData.value
                : undefined,
            seoData,
            systemHealthData.status === "fulfilled"
        ...
        return {
            ...
            webVitals:
                webVitalsData.status === "fulfilled"
                    ? webVitalsData.value
                    : this.getDefaultWebVitalsData(),
            seo: seoData,
            systemHealth:
                systemHealthData.status === "fulfilled"
                    ? systemHealthData.value
                    : this.getDefaultSystemHealthData(),
            quickStats:
                quickStatsData.status === "fulfilled"
                    ? quickStatsData.value
                    : this.getDefaultQuickStatsData(),
     ...
     * 收集Web Vitals数据
     */
    private async collectWebVitalsData(
        timeframe: string,
    ): Promise<PerformanceMetrics["webVitals"]> {
        // 模拟历史数据
        const historicalData: WebVitalsData[] =
            this.generateHistoricalWebVitalsData(timeframe);
        ...
        // 计算趋势
        const trends = this.calculateWebVitalsTrends(
            historicalData,
            currentData,
        );
     ...
     * 收集快速统计数据
     */
    private async collectQuickStatsData(
        timeframe: string,
    ...
    private calculateOverallScores(
        ...
        const healthScore = systemHealth
            ? this.calculateHealthScore(systemHealth)
            : 80;

        const overallScore = Math.round(
            performanceScore * 0.4 + seoScore * 0.3 + healthScore * 0.3,
        );
    ...
    private calculateGrade(
        score: number,
    ): PerformanceMetrics["overview"]["grade"] {
    ...
    private calculateTrend(
        ...
        seoScore: number,
        healthScore: number,
    ): PerformanceMetrics["overview"]["trend"] {
    ...
    private calculateWebVitalsTrends(
        ...
        return {
            lcp:
                current.lcp && recent.lcp
                    ? calculateTrend(current.lcp, recent.lcp)
                    : "stable",
            inp:
                current.inp && recent.inp
                    ? calculateTrend(current.inp, recent.inp)
                    : "stable",
            cls:
                current.cls && recent.cls
                    ? calculateTrend(current.cls, recent.cls)
                    : "stable",
            fcp:
                current.fcp && recent.fcp
                    ? calculateTrend(current.fcp, recent.fcp)
                    : "stable",
            ttfb:
                current.ttfb && recent.ttfb
                    ? calculateTrend(current.ttfb, recent.ttfb)
                    : "stable",
        };
    }
     ...
     * 生成历史Web Vitals数据
     */
    private generateHistoricalWebVitalsData(
        timeframe: string,
    ): WebVitalsData[] {
     ...
     * 转换健康数据格式
     */
    private transformHealthData(healthData: unknown): SystemHealthData {
        const data: HealthDataInput =
            typeof healthData === "object" && healthData !== null
                ? (healthData as HealthDataInput)
                : {};
        ...
        return {
            ...
            components: {
                database: {
                    ...
                    responseTime: data.components?.database?.responseTime ?? 50,
                },
                cache: {
                    status: data.components?.cache?.status ?? "healthy",
                    hitRate: data.components?.cache?.hitRate ?? 85,
                },
                api: {
                    status: data.components?.api?.status ?? "healthy",
                    responseTime: data.components?.api?.responseTime ?? 100,
                },
                storage: {
                    status: data.components?.storage?.status ?? "healthy",
                    available: data.components?.storage?.available ?? 1000,
                },
            },
            timestamp: new Date().toISOString(),
        };
...
export async function getAdminPerformanceMetrics(
    ...
    return await performanceDataService.getPerformanceMetrics(
        params,
        forceRefresh,
    );
}
