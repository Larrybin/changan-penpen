...
export class SEOScanner {
    private cache = new Map<
        string,
        { result: SEOCheckResult; timestamp: number }
    >();
    ...
    async scanPage(url?: string): Promise<SEOCheckResult> {
        ...
        const totalIssues = allChecks.reduce(
        ...
        if (title) {
            ...
            if (length < SEO_THRESHOLDS.title.minLength) {
                ...
                recommendations.push(
                    `标题长度应至少 ${SEO_THRESHOLDS.title.minLength} 字符`,
                );
            } else if (length > SEO_THRESHOLDS.title.maxLength) {
                issues.push(`标题过长 (${length} 字符)`);
                recommendations.push(
                    `标题长度不应超过 ${SEO_THRESHOLDS.title.maxLength} 字符`,
                );
            }

            if (
                length >= SEO_THRESHOLDS.title.minLength &&
                length <= SEO_THRESHOLDS.title.maxLength
            ) {
                score = 100;
                status = "pass";
            } else if (length > 0) {
    ...
    private async checkDescription() {
        ...
        if (description) {
            ...
            if (length < SEO_THRESHOLDS.description.minLength) {
                ...
                recommendations.push(
                    `描述长度应至少 ${SEO_THRESHOLDS.description.minLength} 字符`,
                );
            } else if (length > SEO_THRESHOLDS.description.maxLength) {
                issues.push(`描述过长 (${length} 字符)`);
                recommendations.push(
                    `描述长度不应超过 ${SEO_THRESHOLDS.description.maxLength} 字符`,
                );
            }

            if (
                length >= SEO_THRESHOLDS.description.minLength &&
                length <= SEO_THRESHOLDS.description.maxLength
            ) {
                score = 100;
                status = "pass";
            } else if (length > 0) {
    ...
    private async checkImages() {
        ...
        images.forEach((img) => {
            ...
            if (!alt) {
                ...
                alt.length < SEO_THRESHOLDS.image.minAltLength ||
                alt.length > SEO_THRESHOLDS.image.maxAltLength
            ) {
                imagesWithPoorAlt++;
            }
        });
        ...
        if (imagesWithPoorAlt > 0) {
            ...
            recommendations.push(
                `alt属性长度应在 ${SEO_THRESHOLDS.image.minAltLength}-${SEO_THRESHOLDS.image.maxAltLength} 字符之间`,
            );
        }
    ...
    private async checkLinks() {
        ...
        const _externalLinks = document.querySelectorAll(
