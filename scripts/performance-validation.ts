...
class PerformanceValidator {
            ...
            Object.entries(mockVitals).forEach(([metric, value]) => {
                ...
                if (value <= threshold.good) {
                    ...
                    result.warnings.push(
                        `${metric} 指标需要改进: ${value}${this.getUnit(metric)}`,
                    );
                } else {
                    rating = "poor";
                    result.passed = false;
                    result.errors.push(
                        `${metric} 指标表现不佳: ${value}${this.getUnit(metric)}`,
                    );
                }
    ...
    async checkImageOptimization(): Promise<ValidationResult> {
        ...
        try {
            ...
            this.evaluateNextImageConfig(result);
            this.checkImageAssets(result);
    ...
    async analyzeBundleSize(): Promise<ValidationResult> {
        ...
        try {
            ...
            if (packageJson) {
                this.collectDependencyMetrics(packageJson, result);
                this.checkBundleAnalyzeScript(packageJson, result);
            }
    ...
    private evaluateNextImageConfig(result: ValidationResult): void {
        const nextConfig = this.readTextFile("next.config.ts");
        if (!nextConfig) {
            return;
        }
        ...
        if (nextConfig.includes("formats:")) {
            ...
            const missingFormats = supportedFormats.filter(
                (format) => !nextConfig.includes(format),
            );
    ...
    private checkImageAssets(result: ValidationResult): void {
        const assetChecks: Array<{
            ...
            {
                path: "src/components/ui/image-optimizer.tsx",
                onPresent: () => {
        ...
        assetChecks.forEach((check) => {
            const assetPath = join(this.projectRoot, check.path);
    ...
    private readPackageJson(): Record<string, unknown> | null {
        const content = this.readTextFile("package.json");
        if (!content) {
            return null;
        }
        ...
        try {
            ...
            console.info(`  ⚠️  无法解析 package.json: ${error}`);
            return null;
        }
    }

    private collectDependencyMetrics(
        ...
        const depCount = dependencies ? Object.keys(dependencies).length : 0;
        const devDepCount = devDependencies
    ...
    private checkBundleAnalyzeScript(
        packageJson: Record<string, unknown>,
        result: ValidationResult,
    ): void {
        ...
        if (scripts?.[analyzeScript]) {
            console.info("  ✅ Bundle 分析脚本已配置");
            result.suggestions.push(
                "运行 `pnpm run analyze:bundle` 查看详细的包大小分析",
            );
            return;
        }
    ...
    private readTextFile(relativePath: string): string | null {
    ...
    async runLighthouseSimulation(): Promise<ValidationResult> {
        ...
        try {
            ...
            if (lighthouseScores.performance < 95) {
                result.suggestions.push(
                    "优化图片、减少JavaScript包大小、启用文本压缩",
                );
            }
            if (lighthouseScores.accessibility < 100) {
                result.suggestions.push(
                    "改进颜色对比度、添加ARIA标签、改进键盘导航",
                );
            }
            if (lighthouseScores.seo < 100) {
                result.suggestions.push(
                    "完善meta标签、添加结构化数据、优化页面标题",
                );
            }
        } catch (error) {
