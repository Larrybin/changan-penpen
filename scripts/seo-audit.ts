...
class SEOAuditor {
  ...
  checkSemanticHTML(): AuditResult {
    ...
    try {
      const landingPagePath = join(this.projectRoot, 'src/modules/marketing/landing.page.tsx');
      const headerPath = join(this.projectRoot, 'src/modules/marketing/components/public-header.tsx');
      const footerPath = join(this.projectRoot, 'src/modules/marketing/components/public-footer.tsx');
      ...
      const headerContent = existsSync(headerPath) ? readFileSync(headerPath, 'utf8') : '';
      const footerContent = existsSync(footerPath) ? readFileSync(footerPath, 'utf8') : '';
      ...
      semanticTags.forEach(tag => {
        const count = (landingPage.match(new RegExp(tag, 'g')) || []).length;
        semanticCount += count;
        if (count === 0 && tag !== '<article' && tag !== '<aside') { // article和aside可能不需要
      ...
      // 检查是否有适当的ARIA标签
      const hasAriaLabels = landingPage.includes('aria-') ||
  ...
  checkStructuredData(): AuditResult {
    ...
    try {
      ...
      // 检查JSON-LD实现
      const hasJsonLd = landingPage.includes('application/ld+json') ||
                       landingPage.includes('JSON.stringify(structuredDataPayload)');

      // 检查schema.org结构
      const hasSchemaOrg = landingPage.includes('@context": "https://schema.org"') ||
  ...
  checkResponsiveDesign(): AuditResult {
    ...
    try {
      const globalsCssPath = join(this.projectRoot, 'src/app/globals.css');

      if (!existsSync(globalsCssPath)) {
        result.status = 'warning';
        result.description = '找不到全局CSS文件';
        result.recommendations = ['确保存在全局CSS文件并包含响应式样式'];
        return result;
      }

      const cssContent = readFileSync(globalsCssPath, 'utf8');

      // 检查viewport meta标签（通常在layout中）
      ...
      const hasMobileOptimizations =
        cssContent.includes('min-height: 44px') ||  // 触摸目标
        cssContent.includes('font-size: 16px') ||   // 防止iOS缩放
        cssContent.includes('box-sizing: border-box'); // 防止溢出
      ...
      if (hasMobileOptimizations) responsiveScore++;
      if (hasFluidTypography) responsiveScore++;
      ...
      // 检查容器和布局
      const hasModernLayout = cssContent.includes('display: grid') ||
                             cssContent.includes('display: flex') ||
                             cssContent.includes('container-type');

      if (!hasModernLayout) {
        result.recommendations?.push('考虑使用现代CSS布局技术(Grid/Flexbox)');
      }
  ...
  checkSEOFiles(): AuditResult {
    ...
    try {
      const seoFiles = [
        ...
        { path: 'src/app/sitemap.xml/route.ts', name: 'sitemap.xml路由' },
      ...
      if (existsSync(packageJsonPath)) {
        ...
        hasSEOScripts = Object.keys(scripts).some(script =>
          script.includes('seo') ||
  ...
  checkAccessibility(): AuditResult {
    ...
    try {
      const accessibilityFilePath = join(this.projectRoot, 'src/styles/accessibility.css');
      const skipLinkPath = join(this.projectRoot, 'src/components/accessibility/skip-link.tsx');
