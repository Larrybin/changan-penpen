...
const LOG_PATH = process.env.PUSH_LOG_FILE || path.join(
    LOG_DIR,
...
function timeStart(name) { STEP_TIMES[name] = { start: Date.now(), ms: 0 }; }
function timeEnd(name) { const t = STEP_TIMES[name]; if (t?.start) t.ms = Date.now() - t.start; }
...
function run(cmd, opts = {}) {
    ...
    if (res.stdout) { process.stdout.write(res.stdout); logLine(res.stdout.trimEnd()); }
    if (res.stderr) { process.stderr.write(res.stderr); logLine(res.stderr.trimEnd()); }
...
function checkBom(files) {
    ...
    for (const f of files) {
        try {
            ...
            const b = Buffer.from((() => { try { return readFileSync(f); } catch { return new Uint8Array(0); } })());
...
function getTrackedFiles(globs) {
    try {
        ...
        return (res.stdout || "").split("\u0000").map(s => s.trim()).filter(Boolean);
    } catch { return []; }
}
function getChangedFiles() {
    try {
        const res = spawnSync("git", ["diff","--name-only","--diff-filter=ACMR","--cached"], { encoding: "utf8" });
...
function classifyChanges(files) {
    ...
    const isFlow = (f) => f.startsWith(".github/workflows/") || f.startsWith(".github/actions/");
    const isBind = (f) => f === "wrangler.jsonc" || f === "cloudflare-env.d.ts" || f.startsWith("types/cloudflare-env") || f.startsWith("src/types/cloudflare-env");
    const isCode = (f) => f.startsWith("src/") || f.startsWith("scripts/");
    if (files.length === 0) return { docsOnly: false, workflowsOnly: false, bindingsChanged: true, codeChanged: true };
    return {
...
// --- 1) Typegen + format ----------------------------------------------------
try { tryRun("pnpm exec lint-staged"); } catch {}
if (process.env.SKIP_BOM_CHECK !== "1") {
    const baseList = ["package.json","biome.json","components.json","tsconfig.json","tsconfig.translate.json","wrangler.jsonc"];
...
let DO_CF_TYPEGEN = CH.bindingsChanged;
let DO_TSC = !CH.docsOnly && !CH.workflowsOnly;
...
// --- 2) Type check ----------------------------------------------------------
if (DO_TSC) {
    try { if (existsSync(".next")) rmSync(".next", { recursive: true, force: true }); } catch {}
    timeStart("tsc");
...
if (status) {
    ...
    if (process.env.PUSH_COMMIT_MSG_FILE) {
        ...
        const files = getOutput("git diff --cached --name-only").split(/\r?\n/).filter(Boolean);
        const scopeBits = new Set();
        if (files.some(f => f.startsWith("src/"))) scopeBits.add("code");
        if (files.some(f => f.startsWith("docs/") || f === "README.md")) scopeBits.add("docs");
        if (files.some(f => f.startsWith(".github/workflows/") || f.startsWith(".github/actions/"))) scopeBits.add("workflows");
...
// --- 6) Rebase & push -------------------------------------------------------
{
    ...
    let conflictList = "";
    try { conflictList = getOutput("git diff --name-only --diff-filter=U"); } catch {}
    if (!pulled || (conflictList && conflictList.trim().length > 0)) {
        ...
        console.error(hint); logLine(hint);
        process.exit(1);
    }
...
try {
    ...
    if (STEP_TIMES["cf-typegen"]) console.log(`- cf-typegen: ${CF_TYPEGEN ? "OK" : "FAILED"} (${STEP_TIMES["cf-typegen"].ms}ms)`);
    console.log(`- Lint (Biome write): ${BIOME_WRITE_RAN ? "Ran" : "Skipped"}` + (STEP_TIMES["biome-write"] ? ` (${STEP_TIMES["biome-write"].ms}ms)` : ""));
    console.log(`- TypeScript: ${TSC_OK ? "OK" : (STEP_TIMES["tsc"] ? "FAILED" : "Skipped")}` + (STEP_TIMES["tsc"] ? ` (${STEP_TIMES["tsc"].ms}ms)` : ""));
    console.log(`- Unit tests: ${TEST_OK ? "OK" : (process.env.SKIP_TESTS === "1" ? "Skipped" : (STEP_TIMES["vitest"] ? "FAILED" : "Skipped"))}` + (STEP_TIMES["vitest"] ? ` (${STEP_TIMES["vitest"].ms}ms)` : ""));
    console.log(`- Docs consistency: ${DOCS_OK ? "OK" : "FAILED/Skipped"}`);
    console.log(`- Link check: ${LINKS_OK ? "OK" : "FAILED/Skipped"}`);
    console.log(`- Semgrep: ${SEMGREP_OK ? "OK" : process.env.SKIP_SEMGREP === "1" ? "Skipped" : "FAILED"}`);
    console.log(`- Biome final: ${BIOME_FINAL_OK ? "OK" : "FAILED"}` + (STEP_TIMES["biome-final"] ? ` (${STEP_TIMES["biome-final"].ms}ms)` : ""));
    if (STEP_TIMES["docs-checks"]) console.log(`- Docs/links time: ${STEP_TIMES["docs-checks"].ms}ms`);
    if (STEP_TIMES["next-build"]) console.log(`- Next.js build time: ${STEP_TIMES["next-build"].ms}ms`);
