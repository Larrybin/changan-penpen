...
for (const file of walk(ROOT)) {
  ...
  function typographicApostrophes(value) {
    ...
    return value.replace(/([A-Za-zÀ-ÖØ-öø-ÿ])'([A-Za-zÀ-ÖØ-öø-ÿ])/g, "$1’$2");
  }
  ...
  function nbspNumberUnit(value) {
    if (typeof value !== "string") return value;
    let out = value.replace(/(\d)\s+(MP|KB|K[Bb]|MB|GB|TB|%|km|cm|mm|px)/g, "$1\u00A0$2");
    // Placeholder-based: {{total}} {{unit}} -> {{total}} NBSP {{unit}}
    out = out.replace(/(\{\{\s*(?:total|count|amount|size)\s*\}\})\s+(\{\{\s*unit\s*\}\})/g, "$1\u00A0$2");
    return out;
  }
  ...
  function walkDashes(obj, locale) {
    ...
    const next = locale === "de" ? normalizeDashesDE(obj) : locale === "pt" ? normalizeDashesPT(obj) : obj;
    if (next !== obj) return next;
    return obj;
  }
  ...
  if (locale === "fr") {
    ...
    function applyFrenchSpacing(value) {
      ...
      let replaced = value.replace(/[\u00A0\u202F ]+([!?;:])/g, "\u202F$1")
        // And ensure there is exactly one U+202F before the punctuation when there was none
        .replace(new RegExp(`([^\u00A0\u202F\s])(${FRENCH_PUNCT_BEFORE.source})`, "g"), "$1\u202F$2");
