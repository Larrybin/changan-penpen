...
function getChangedFiles() {
  try {
    const r = spawnSync("git", ["diff", "--name-only", "--diff-filter=ACMR", "--cached"], { encoding: "utf8" });
  ...
  try {
    ...
    if (base) {
      try { spawnSync("git", ["fetch", "origin", base, "--depth=1"], { stdio: "ignore" }); } catch {}
  ...
  try {
    const r = spawnSync("git", ["diff", "--name-only", "HEAD~1"], { encoding: "utf8" });
    const a = (r.stdout || "").split(/[\r\n]+/).filter(Boolean);
    if (a.length) return a;
  } catch {}
  return [];
...
function gatherEnvBindings() {
  ...
  if (exists(wrangler)) {
    try {
      ...
      if (Array.isArray(json?.d1_databases)) for (const x of json.d1_databases) b.push({ type: "D1", name: x?.binding, database_id: x?.database_id });
      if (Array.isArray(json?.r2_buckets)) for (const x of json.r2_buckets) b.push({ type: "R2", name: x?.binding, bucket_name: x?.bucket_name });
      if (Array.isArray(json?.kv_namespaces)) for (const x of json.kv_namespaces) b.push({ type: "KV", name: x?.binding, id: x?.id });
      if (Array.isArray(json?.queues?.producers)) for (const x of json.queues.producers) b.push({ type: "QueueProducer", name: x?.binding, queue: x?.queue });
      if (Array.isArray(json?.queues?.consumers)) for (const x of json.queues.consumers) b.push({ type: "QueueConsumer", queue: x?.queue });
      if (json?.ai) b.push({ type: "AI", name: typeof json.ai === "string" ? json.ai : "AI" });
      if (json?.assets) b.push({ type: "ASSETS", name: json.assets?.binding || "ASSETS" });
      out.bindings = b;
      out.vars = a;
    } catch {}
  }
  ...
  if (bindings.length) {
    ...
    for (const b of bindings) {
      const detail = Object.entries(b)
        ...
        .map(([k, v]) => `${k}=${v ?? ""}`)
        .join(", ");
      lines.push(`| ${b.type} | ${b.name ?? b.queue ?? ""} | ${detail} |`);
    }
  } else {
...
function listWorkflows() {
  ...
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    out.push({ file: rel(p), name, triggers: Array.from(new Set(triggers)) });
  }
...
function renderWorkflowTable() {
  ...
  const lines = ["### Workflows Overview (auto)", "| Workflow | Triggers | File |", "| --- | --- | --- |"];
  for (const x of w) lines.push(`| ${x.name} | ${x.triggers.join(", ")} | ${x.file} |`);
...
function readPackageJson() {
  ...
  try { return JSON.parse(readUtf8(p)); } catch { return { scripts: {} }; }
}

function renderScriptsTable() {
  const { scripts } = readPackageJson();
  const names = Object.keys(scripts || {}).sort();
  const lines = ["### Common Scripts (auto)", "| Script | Command |", "| --- | --- |"];
...
function renderReadmeAutomation() {
  ...
  lines.push("- Local push integrates docs sync/autogen, lint/typecheck/tests, optional Next build, and rebase & push.");
  lines.push("- No extra commits: changes are amended into the last commit. Set `ALLOW_FORCE_PUSH=1` to handle non-fast-forward push after amend.");
  lines.push("- See more: docs/local-dev.md, docs/ci-cd.md, docs/docs-maintenance.md");
  ...
  if (top.length) {
    ...
    lines.push("| --- | --- | --- |");
    for (const x of top) lines.push(`| ${x.name} | ${x.triggers.join(", ")} | ${x.file} |`);
    if (w.length > top.length) lines.push(`… and ${w.length - top.length} more (see docs/ci-cd.md).`);
  } else {
...
function renderReadmeStructure() {
  ...
  try {
    ...
    if (exists(agents)) {
      ...
      const secRe = /^##\s+Project Structure\s*&\s*Module Organization\s*$/mi;
      ...
      if (m) {
        ...
        for (const ln of linesMd) {
          if (!started) { if (/^\s*-\s+/.test(ln)) started = true; else continue; }
  ...
  const paths = [
    { p: "src/app", note: "Next.js App Router (pages/api routes, layouts)" },
    { p: "src/modules", note: "Domain modules (actions/components/hooks/models/schemas/utils)" },
  ...
  if (exists(modsDir)) {
    try {
      const mods = fs.readdirSync(modsDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name).sort();
      const show = mods.slice(0, 10).join(", ");
      if (mods.length) outLines.push("- Modules: " + show + (mods.length > 10 ? " …(+" + (mods.length - 10) + ")" : ""));
    } catch {}
  }
...
function renderReadmeQuality() {
  ...
  try {
    ...
    if (exists(q)) {
      ...
      if (idxLocal >= 0) {
        ...
        if (idxExec >= 0) {
          const tail2 = tail.slice(idxExec + "execution order:".length);
        ...
        const idxTog = tail.toLowerCase().indexOf("environment toggles:");
        if (idxTog >= 0) {
          const tail3 = tail.slice(idxTog + "environment toggles:".length);
          ...
          if (toggles.length) {
            ...
            for (const l of toggles.slice(0, 10)) lines.push("  " + l);
          }
        }
      }
    }
  ...
  lines.push("- No extra commits: changes are amended into the last commit (ALLOW_FORCE_PUSH=1 for non-fast-forward).");
  return lines.join("\n");
}
...
function pageFromRouteFile(p) {
  ...
  return "/" + seg.replace(/\\+/g, "/").replace(/(^|\/)\(.*?\)\/?/g, "$1").replace(/\/$/, "");
}
...
function renderApiIndex() {
  ...
  const files = walk(appDir, (p) => /\/route\.ts$/.test(p.replace(/\\/g, "/"))).sort();
  const rows = [];
  ...
  const lines = ["### API Index (auto)", "| Path | Methods | File |", "| --- | --- | --- |"];
  for (const r of rows) lines.push(`| ${r.path} | ${r.methods.join(", ")} | ${r.file} |`);
...
function main() {
  ...
  const shouldRun = (paths) => {
    ...
    return paths.some((p) => Array.from(changed).some((c) => c.startsWith(p)));
  };
  ...
  if (shouldRun(["wrangler.jsonc", ".dev.vars.example"])) {
    ...
    const updated = applyToFile("docs/env-and-secrets.md", [["ENV_BINDINGS", body]]);
    if (updated != null && !dry) writeUtf8(path.join(root, "docs/env-and-secrets.md"), updated);
  ...
  if (shouldRun([".github/workflows/"])) {
    ...
    const updated = applyToFile("docs/ci-cd.md", [["WORKFLOWS_TABLE", body]]);
    if (updated != null && !dry) writeUtf8(path.join(root, "docs/ci-cd.md"), updated);
  ...
  if (shouldRun(["package.json"])) {
    ...
    const updated = applyToFile("docs/local-dev.md", [["SCRIPTS_TABLE_AUTO", body]]);
    if (updated != null && !dry) writeUtf8(path.join(root, "docs/local-dev.md"), updated);
  ...
  if (shouldRun(["src/app/"])) {
    ...
    if (updated != null && !dry) writeUtf8(path.join(root, "docs/api-index.md"), updated);
    if (updated != null) results.push("docs/api-index.md");
  }

  // README snapshots
  if (shouldRun(["package.json", ".github/workflows/", "scripts/", "src/", "docs/quality-gates.md", "AGENTS.md"])) {
    const sections = [
    ...
    if (updated != null && !dry) writeUtf8(path.join(root, "README.md"), updated);
  ...
  try { writeUtf8(outFile, JSON.stringify({ scope, dryRun: dry, changed: results }, null, 2)); } catch {}
  console.log(`[docs-autogen] Updated ${results.length} file(s).` + (dry ? " (dry-run)" : ""));
}
