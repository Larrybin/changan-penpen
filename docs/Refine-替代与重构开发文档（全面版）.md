...
**阶段 C 完成标志**： 项目完全摆脱 Refine，运行稳定，性能达标，测试覆盖率达标，并且团队对新技术栈应用熟练。此时整个替代与重构工作宣告完成。
...
- **CI 集成**： 持续集成流程需做相应调整。在 `.github/workflows/deploy.yml` 和 `ci.yml` 中增加步骤：
  - **测试**：增加 Vitest 覆盖率执行（`pnpm test -- --coverage`），并上传结果作为构件以供审核。
  - **质控**：利用 SonarQube 或 Codecov 等，持续跟踪覆盖率历史。必要时可以设置质量门禁，比如“凡低于目标覆盖率的 PR 需要审批豁免”。
  - **OpenAPI 合规**：在部署前，运行 `pnpm openapi:generate` 并比对 `public/openapi.json` 有无变更，确保前端所用的 Zod schema 与 OpenAPI 同步。如果不一致应阻止部署以防契约漂移。
  - **健康检查**：部署完毕后，Pipeline 可调用 `/api/v1/health?fast=1` 做快速健康检查，若失败则回滚部署\[37\]。也可结合 Cloudflare API 检查 Workers 日志无大量异常再判定成功。现有 `/api/v1/health` 机制需延续，重构不应改变其行为（如仍支持 `fast=1` 参数、`X-Health-Token` 鉴权等\[38\]）。
  - **验收标准与 PR 模板**： 为确保替换效果达到预期，可在每个功能模块重构完成后安排验收会议或演示，使用预先定义的验收标准逐项核对。例如：
    - 导航菜单：项数正确、名称正确、权限隐藏正确、链接正确（人工点击验证）。
    - 列表页：默认加载数据、分页是否工作、筛选是否生效、页面切换无报错。
    - 表单页：提交成功/失败路径，错误消息展示位置等。
    - 性能：首次加载和主要交互耗时与之前相当，无明显退化。

  建议制定一个 PR 模板，在描述中要求填写 “本次变更是否通过所有列表等价清单测试”、“截图/录屏证据”、“新增测试清单”等。虽然用户不会看到这些，但团队内部流程会因此更严格，把好每一关。

通过如上多层次的测试和质量控制，我们有信心在重构过程中及时发现和纠正问题，保证最终上线的系统质量达标甚至超出原有水平\[39\]（Refine 官方重构最佳实践亦强调以测试验证代码质量提升）。
...
- **CI 构建流程**： 保持使用 OpenNext 的 build 命令（`opennextjs-cloudflare build`）和 Wrangler 部署。重构不会改变这一流程，但是要更新 **构建前置步骤**：在 Build 之前，加入前述 OpenAPI 文档生成、Vitest 测试步骤，并确保 `pnpm build` 产物包含最新类型定义和文档。例如，可以在 workflow 中这样顺序： Test -> OpenAPI Generate -> Build -> Deploy。如果 generate 发现未提交的 API 文档变更，则可以视为错误提示开发者更新。
...
- **CI/CD 门禁**： 最后，将提高的测试覆盖率要求等设置在 CI 的保护规则中，使之长期生效。也就是在未来的每个 Pull Request 中，都强制保持我们重构后的高标准，不让质量下滑。这将巩固重构收益，也是 DevOps 治理的一部分\[43\]（很多成熟企业在 CI 中要求 50% 以上覆盖才能通过，我们 80% 已属高标准）。
